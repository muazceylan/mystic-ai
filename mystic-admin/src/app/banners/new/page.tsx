...
export default function NewBannerPage() {
  ...
  const [form, setForm] = useState({ bannerKey: '', placementType: 'HOME_HERO', title: '', subtitle: '', imageUrl: '', ctaLabel: '', routeKey: '', fallbackRouteKey: '', priority: 0, locale: 'tr', startDate: '', endDate: '' });
  const [error, setError] = useState('');

  const createMut = useMutation({
    mutationFn: (data: typeof form) => bannersApi.create(data),
    onSuccess: (r) => router.push(`/banners/${r.data.id}`),
  ...
  const inp = (key: keyof typeof form, label: string, type = 'text', opts?: string[]) => (
    <div>
      ...
      {opts ? (
        <select value={form[key] as string} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} className="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-3 py-2 text-sm">
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-white">New Banner</h1>
      </div>
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          {inp('bannerKey', 'Banner Key *')}
          {inp('placementType', 'Placement Type', 'text', PLACEMENTS)}
          {inp('title', 'Title *')}
          {inp('subtitle', 'Subtitle')}
          {inp('imageUrl', 'Image URL *')}
          {inp('ctaLabel', 'CTA Label')}
          {inp('priority', 'Priority', 'number')}
          {inp('locale', 'Locale', 'text', ['tr', 'en'])}
