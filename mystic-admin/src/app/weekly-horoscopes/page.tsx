...
import { WeeklyHoroscope, IngestStatus, Page } from '@/types';
...
import { Plus, RefreshCw, Download, Clock, Database, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function WeeklyHoroscopesPage() {
  ...
  const [ingestWeekStart, setIngestWeekStart] = useState(currentWeekMonday());
  ...
  const { data, isLoading } = useQuery<Page<WeeklyHoroscope>>({
    queryKey: ['weekly-horoscopes', params],
    queryFn: () => weeklyHoroscopeApi.list(params).then((r) => r.data),
  });

  const { data: ingestStatus } = useQuery<IngestStatus>({
    queryKey: ['weekly-horoscopes-ingest-status', locale],
    queryFn: () => weeklyHoroscopeApi.ingestStatus(locale).then((r) => r.data),
  });

  const ingestMut = useMutation({
    mutationFn: () => weeklyHoroscopeApi.ingest(ingestSign, ingestWeekStart, ingestLocale),
    onSuccess: () => {
  ...
  const publishMut = useMutation({
    mutationFn: (id: number) => weeklyHoroscopeApi.publish(id),
    onSuccess: () => {
  ...
  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-white">Haftalık Burç İçerikleri</h1>
        <div className="flex gap-2">
          <Button variant="secondary" onClick={() => setIngestModal(true)} size="sm">
            <Download className="w-4 h-4 mr-1" /> İngest Et
          </Button>
          <Link href="/weekly-horoscopes/new">
            <Button size="sm"><Plus className="w-4 h-4 mr-1" /> Yeni</Button>
          </Link>
        </div>
      </div>
            ...
            <div>
              ...
              <p className="text-sm text-white font-medium">
                {ingestStatus.lastIngestDate
                  ? new Date(ingestStatus.lastIngestDate).toLocaleDateString('tr-TR', {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-800 text-gray-400 text-xs uppercase">
              ...
              <th className="px-4 py-3 text-left">Hafta</th>
          ...
          <tbody>
            ...
            {data?.content.map((h) => (
              <tr key={h.id} className={`border-b border-gray-800 hover:bg-gray-800/50 ${h.ingestError ? 'bg-red-950/20' : ''}`}>
                ...
                <td className="px-4 py-3">
                  {h.isOverrideActive
                    ? <span className="text-amber-400 text-xs font-medium">Aktif</span>
                    : <span className="text-gray-600 text-xs">-</span>}
                </td>
                <td className="px-4 py-3 text-gray-400">{formatDate(h.updatedAt)}</td>
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <Link href={`/weekly-horoscopes/${h.id}`}>
      ...
      {ingestModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 w-full max-w-sm space-y-4">
            <h3 className="text-white font-semibold">Astroloji Servisinden Haftalık İçerik Çek</h3>
            <div>
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1">Hafta Başlangıcı (Pazartesi)</label>
              <input type="date" value={ingestWeekStart} onChange={(e) => setIngestWeekStart(e.target.value)}
