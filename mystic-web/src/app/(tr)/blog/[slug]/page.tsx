...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post = await fetchPostBySlug(slug, 'tr');
  if (!post) return {};
  ...
  return {
    ...
    alternates: getPostMetadataAlternates(post.slug, 'tr', availableLocales),
    openGraph: {
...
const categoryLinks: Record<string, { href: string; label: string; featureName: string }> = {
  astroloji: { href: '/astroloji', label: 'Astroloji', featureName: 'astrology' },
  numeroloji: { href: '/numeroloji', label: 'Numeroloji', featureName: 'numerology' },
  ruya: { href: '/ruya-yorumu', label: 'Ruya Yorumu', featureName: 'dream_interpretation' },
  spirituel: { href: '/spirituel-rehberlik', label: 'Spirituel Rehberlik', featureName: 'spiritual_guidance' },
};
...
export default async function BlogPostPage({ params }: Props) {
  ...
  return (
    <>
      <JsonLd data={articleJsonLd(post, 'tr')} />
      <ArticleOpenTracker
        slug={post.slug}
        title={post.title}
      ...
      <article className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
        ...
        <div className="mt-12 rounded-2xl bg-purple-950 p-8 text-center text-white">
          <h2 className="text-xl font-bold">AstroGuru ile Kesfet</h2>
          <p className="mt-2 text-purple-200">
            Uygulamayi indir ve kisisel kozmik rehberligini al.
          </p>
          <DownloadCtaLink
            href="/#indir"
        ...
        {relatedLink && (
          <div className="mt-8 text-center">
            ...
            >
              {relatedLink.label} hakkinda daha fazla bilgi &rarr;
            </TrackedLink>
          </div>
        )}
      </article>
