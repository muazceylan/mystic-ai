...
const MODULE_KEY = 'share_cards';
const ACTION_KEY_EXPORT = 'share_card_export';
...
export default function ShareCardPreviewScreen() {
   ...
   */
  const checkMonetizationGate = useCallback((): boolean => {
    const exportAction = monetization.getAction(ACTION_KEY_EXPORT);
    if (!exportAction || !monetization.guruEnabled) return true;

    if (monetization.canAffordAction(ACTION_KEY_EXPORT)) {
      MonetizationEvents.gateSeen(MODULE_KEY, ACTION_KEY_EXPORT, 'guru_spend');
      setShowGuruModal(true);
      return false;
    }
    if (monetization.shouldShowAd && monetization.adsEnabled) {
      MonetizationEvents.gateSeen(MODULE_KEY, ACTION_KEY_EXPORT, 'ad');
      setShowAdOffer(true);
      return false;
    }
    // No gate configured or can't show — proceed
    return true;
  }, [monetization]);

  const handleShare = useCallback(async () => {
    if (!checkMonetizationGate()) return;
  ...
  return (
    ...
    >
          ...
          {showAdOffer && monetization.adsEnabled && (
            <View style={{ marginTop: spacing.sm }}>
              <AdOfferCard
                ...
                onComplete={() => setShowAdOffer(false)}
                onDismiss={() => setShowAdOffer(false)}
              />
            </View>
          )}
      ...
      <GuruUnlockModal
        ...
        onShowPurchase={monetization.isActionPurchaseAllowed(ACTION_KEY_EXPORT) ? () => {
          setShowGuruModal(false);
