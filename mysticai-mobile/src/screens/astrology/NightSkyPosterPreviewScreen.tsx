...
import {
  createWebShareFileSync,
  ShareServiceError,
  instagramStory,
  saveToGallery,
  shareImage,
  sharePreparedWebImage,
...
export default function NightSkyPosterPreviewScreen() {
  ...
  const webPosterCaptureRef = useRef<View | null>(null);
  ...
  const [webPosterViewportWidth, setWebPosterViewportWidth] = useState(0);
  const [webImageUri, setWebImageUri] = useState<string | null>(null);
  const [webCaptureLoading, setWebCaptureLoading] = useState(false);
  ...
  const webCaptureKeyRef = useRef<string | null>(null);
  const webCaptureSeqRef = useRef(0);
  ...
  const shareTarget = useMemo(
    () => normalizePublicWebUrl(resolvedShareUrl ?? draft?.shareUrl, getUniversalDownloadUrl('/dl?utm_source=night_sky_poster')),
    [draft?.shareUrl, resolvedShareUrl],
  );
  ...
  const webPosterScale = useMemo(() => {
    if (!webPosterViewportWidth) return 1;
    return webPosterViewportWidth / posterTokens.frame.width;
  }, [webPosterViewportWidth]);
  ...
  }, [draft, layoutTick, renderKey, generate, isWeb]);

  useEffect(() => {
    if (!isWeb) return;
    webCaptureSeqRef.current += 1;
    webCaptureKeyRef.current = null;
    setWebImageUri(null);
    setWebCaptureLoading(false);
  }, [isWeb, renderKey]);

  const captureWebPosterImage = useCallback(async () => {
    if (!webPosterCaptureRef.current) {
      throw new Error(t('nightSkyPosterPreview.regenerateErrorMessage'));
    }

    if (renderKey && webCaptureKeyRef.current === renderKey && webImageUri) {
      return webImageUri;
    }

    const captureSeq = webCaptureSeqRef.current + 1;
    webCaptureSeqRef.current = captureSeq;
    setWebCaptureLoading(true);

    try {
      const html2canvas = require('html2canvas') as (element: any, options?: any) => Promise<any>;
      await new Promise<void>((resolve) => setTimeout(resolve, 120));
      const canvas = await html2canvas(webPosterCaptureRef.current, {
      ...
      if (webCaptureSeqRef.current === captureSeq) {
        webCaptureKeyRef.current = renderKey;
        setWebImageUri(uri);
      }
      return uri;
    } finally {
  ...
  }, [renderKey, t, webImageUri]);

  useEffect(() => {
    if (!isWeb || !draft || !posterModel || !renderKey || webPosterViewportWidth <= 0) return;
    if (webCaptureKeyRef.current === renderKey && webImageUri) return;

    const timer = setTimeout(() => {
      void captureWebPosterImage().catch(() => {
        // handled lazily during share actions
      });
    }, 160);

    return () => clearTimeout(timer);
  }, [captureWebPosterImage, draft, isWeb, posterModel, renderKey, webImageUri, webPosterViewportWidth]);

  const capturePosterImage = useCallback(async () => {
    if (isWeb) {
      return captureWebPosterImage();
    }

    if (imageUri) return imageUri;
    return generate();
  }, [captureWebPosterImage, generate, imageUri, isWeb]);
  const actionButtonsBusy = loading || actionLoading != null || (isWeb && webCaptureLoading && !webImageUri);
  ...
  const handleShare = async () => {
    await runShareAction('share', t('nightSkyPosterPreview.shareErrorTitle'), async () => {
      const shareTitle = draft?.isGuest
        ? t('nightSkyPosterPreview.pdfTitle')
        : t('nightSkyPosterPreview.pdfTitleWithName', { name: draft?.fullName ?? draft?.name ?? 'Astro Guru' });

      if (isWeb && webImageUri) {
        const preparedFile = createWebShareFileSync(webImageUri, 'night-sky-poster.png');
        if (preparedFile) {
          try {
            const result = await sharePreparedWebImage(preparedFile, {
              title: shareTitle,
              message: t('nightSkyPosterPreview.headerSubtitle'),
            });

            await notifySuccess(
              result.fallbackUsed
                ? t('nightSkyPosterPreview.successSystemShare')
                : t('nightSkyPosterPreview.successShare'),
            );
            return;
          } catch {
            // Some desktop browsers reject file-based Web Share.
            // Fall through to the generic image share/download flow.
          }
        }
      }
      ...
      const result = await shareImage(posterUri, {
        title: shareTitle,
        message: t('nightSkyPosterPreview.headerSubtitle'),
        fileName: 'night-sky-poster.png',
        preferFileOnly: isWeb,
      });

      if (result.message === 'DOWNLOADED' || result.message === 'DOWNLOADED_AND_COPIED') {
        await notifySuccess(t('nightSkyPosterPreview.successDownload'));
        return;
      }
  ...
  const handleInstagram = async () => {
    await runShareAction('instagram', t('nightSkyPosterPreview.instagramErrorTitle'), async () => {
      const posterUri = await capturePosterImage();
      const result = await instagramStory(posterUri, {
        attributionURL: shareTarget,
  ...
  const handleSave = async () => {
    await runShareAction('save', t('nightSkyPosterPreview.saveErrorTitle'), async () => {
      const posterUri = await capturePosterImage();
      const result = await saveToGallery(posterUri);
      await notifySuccess(
        result.message === 'DOWNLOADED'
          ? t('nightSkyPosterPreview.successDownload')
          : (result.message ?? t('nightSkyPosterPreview.successSave')),
      );
    });
  };
        ...
        <View style={[styles.previewShell, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          {loading ? (
            ...
            <>
              {isWeb ? (
                <View
                  style={styles.webPosterViewport}
                  onLayout={(event) => setWebPosterViewportWidth(event.nativeEvent.layout.width)}
                >
                  <View
                    ref={webPosterCaptureRef}
                    style={[
                      styles.webPosterScale,
                      {
                        width: posterTokens.frame.width,
                        height: posterTokens.frame.height,
                        transform: [{ scale: webPosterScale }],
                      },
                    ]}
                  >
                    <BirthNightSkyPoster
