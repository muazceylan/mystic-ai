...
const TOGGLE_ITEMS: ToggleItem[] = [
  { id: 'dailyEnabled', titleKey: 'notifSettings.dailyEnabled', subKey: 'notifSettings.dailyEnabledSub', icon: 'sunny-outline', section: 'main' },
  { id: 'intradayEnabled', titleKey: 'notifSettings.intradayEnabled', subKey: 'notifSettings.intradayEnabledSub', icon: 'time-outline', section: 'main' },
  { id: 'weeklyEnabled', titleKey: 'notifSettings.weeklyEnabled', subKey: 'notifSettings.weeklyEnabledSub', icon: 'calendar-outline', section: 'main' },
  { id: 'dreamReminderEnabled', titleKey: 'notifSettings.dreamReminder', subKey: 'notifSettings.dreamReminderSub', icon: 'moon-outline', section: 'reminder' },
  { id: 'prayerReminderEnabled', titleKey: 'notifSettings.prayerReminder', subKey: 'notifSettings.prayerReminderSub', icon: 'heart-outline', section: 'reminder' },
  { id: 'meditationReminderEnabled', titleKey: 'notifSettings.meditationReminder', subKey: 'notifSettings.meditationReminderSub', icon: 'leaf-outline', section: 'reminder' },
  { id: 'plannerReminderEnabled', titleKey: 'notifSettings.plannerReminder', subKey: 'notifSettings.plannerReminderSub', icon: 'calendar-outline', section: 'reminder' },
  { id: 'eveningCheckinEnabled', titleKey: 'notifSettings.eveningCheckin', subKey: 'notifSettings.eveningCheckinSub', icon: 'cloudy-night-outline', section: 'reminder' },
  { id: 'productUpdatesEnabled', titleKey: 'notifSettings.productUpdates', subKey: 'notifSettings.productUpdatesSub', icon: 'sparkles-outline', section: 'other' },
  { id: 'pushEnabled', titleKey: 'notifSettings.pushEnabled', subKey: 'notifSettings.pushEnabledSub', icon: 'notifications-outline', section: 'other' },
];
...
function makeStyles(C: ThemeColors) {
  return StyleSheet.create({
    ...
    optionBtnSelected: { borderColor: C.primary, backgroundColor: C.primarySoftBg },
    ...
    modalTitle: { fontSize: 15, fontWeight: '600', color: C.text, marginBottom: 20 },
    pickerRow: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 24 },
    ...
    pickerValue: { fontSize: 32, fontWeight: '700', color: C.text, fontVariant: ['tabular-nums'] },
    pickerSep: { fontSize: 28, fontWeight: '700', color: C.subtext, marginTop: 4 },
    modalSaveBtn: { paddingVertical: 10, paddingHorizontal: 40, borderRadius: 10, backgroundColor: C.primary },
      ...
      try {
        if (key === 'pushEnabled' && Platform.OS !== 'web') {
          if (value) {
            ...
            const tokenData = await Notifications.getExpoPushTokenAsync({ projectId: EXPO_PUSH_PROJECT_ID });
  ...
  return (
    <SafeScreen disableBottomTabBarCompensation>
      <View style={styles.container}>
          ...
          <View style={styles.card}>
            ...
            >
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                <Text style={styles.quietValue}>{preferences?.quietHoursStart ?? '22:30'}</Text>
            ...
            >
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                <Text style={styles.quietValue}>{preferences?.quietHoursEnd ?? '08:00'}</Text>
                <Ionicons name="chevron-forward" size={14} color={colors.subtext} />
              </View>
            </Pressable>
          </View>
        ...
        >
          <Pressable style={styles.modalOverlay} onPress={() => setQuietPickerVisible(false)}>
            <Pressable style={styles.modalContent} onPress={() => {}}>
              <Text style={styles.modalTitle}>
                {quietPickerField === 'start' ? t('notifSettings.quietStart') : t('notifSettings.quietEnd')}
              </Text>
              <View style={styles.pickerRow}>
                <View style={styles.pickerColumn}>
                  <Pressable onPress={() => setQuietPickerHour((h) => (h + 1) % 24)} style={styles.pickerArrow}>
                    <Ionicons name="chevron-up" size={22} color={colors.text} />
                  </Pressable>
                  <Text style={styles.pickerValue}>{String(quietPickerHour).padStart(2, '0')}</Text>
                  <Pressable onPress={() => setQuietPickerHour((h) => (h - 1 + 24) % 24)} style={styles.pickerArrow}>
                    <Ionicons name="chevron-down" size={22} color={colors.text} />
                  </Pressable>
                </View>
                <Text style={styles.pickerSep}>:</Text>
                <View style={styles.pickerColumn}>
                  <Pressable onPress={() => setQuietPickerMinute((m) => (m + 15) % 60)} style={styles.pickerArrow}>
                    <Ionicons name="chevron-up" size={22} color={colors.text} />
                  </Pressable>
                  <Text style={styles.pickerValue}>{String(quietPickerMinute).padStart(2, '0')}</Text>
                  <Pressable onPress={() => setQuietPickerMinute((m) => (m - 15 + 60) % 60)} style={styles.pickerArrow}>
                    <Ionicons name="chevron-down" size={22} color={colors.text} />
                  </Pressable>
                </View>
              </View>
