...
public class SecurityStartupValidator implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        String normalizedOpenAiKey = normalize(openAiApiKey);
        if (normalizedOpenAiKey.isBlank() || DEFAULT_OPENAI_API_KEY.equals(normalizedOpenAiKey)) {
            throw new IllegalStateException(
                    "OPENAI_API_KEY must be set to a non-default value outside local environment."
            );
        }
